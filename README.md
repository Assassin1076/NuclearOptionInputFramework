...
其中:
1. Action的名字作为唯一标识符
2. 结构"YourMod::YourAction"仅为推荐，你可以使用任意独特的字符串，若字符串重复，框架将跳过并不注册你的Action
3. category为可选，默认将归类为Debug。由于游戏本身的结构问题，建议不要指定，若指定，请保证指定的category存在

框架会在Rewired启动时一次性注册全部Action到Rewired。
`存在一些Corner Case，你的Mod向框架注册Action时Rewired已经启动`，
